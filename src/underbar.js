(function() {
  ...
  _.uniq = function(array, isSorted, iterator) {
    ...
    //create uniq array and mapped array
    //if iterator is not specified, then set iterator to identity
    //for each element in array
    //check value array if it exists in there
    //if not, push iterator(item) to uniq array and value
    //return uniq array 
    ...
    _.each(array, function(item) {
        var mappedValue = iterator(item);
        if (!mappedArray.includes(mappedValue)) {
  ...
  _.reduce = function(collection, iterator, accumulator) {
    //if accumulator doesn't exist, accumulator flag, set on 2nd part
      
      //set it to collection[0]
